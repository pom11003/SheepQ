...
- フロントエンドからバックエンドAPIへは HTTP 通信により`http://localhost:3001` を参照する構成とする
...
本章では、本プロジェクトで実際に行ったセキュリティ対策および非機能要件への対応をまとめる。

---

### 9.1 認証・認可

- 認証方式として JWT（JSON Web Token）を採用
- フロントエンドからのAPIリクエストには、以下の形式でトークンを付与する

```
Authorization: Bearer <JWT>
```

- Rails側ではリクエストごとにJWTを検証し、
  - 未認証の場合は 401 Unauthorized
  - 管理者権限が必要な場合は 403 Forbidden
    を返却する
- 管理画面（`/admin` 配下）は `role = admin` のユーザーのみ許可する

---

### 9.2 入力値検証・バリデーション

- Strong Parameters により、許可したパラメータのみを受け付ける
- 管理者クイズ作成・更新時には以下の制約をAPI側で必ず検証する
  - choices は **4つ固定**
  - is_correct は **ちょうど1つ**

不正な入力の場合は 422 Unprocessable Entity を返す。

---

### 9.3 SQL Injection 対策

- SQL Injection 対策
  - ActiveRecord を利用し、生SQLを直接組み立てない
  - Strong Parameters により不正な条件指定を防止する

---

### 9.4 トランザクション管理

- クイズ作成・更新時は `ActiveRecord::Base.transaction` を使用し、
  - quiz と choices を **必ず同時に整合性のある状態で保存**する
  - 途中でエラーが発生した場合はロールバックする
...
- 以下の値は環境変数で管理し、git管理しない
  - `JWT_SECRET`
  - `DATABASE_URL`
- ローカルでは `.env`、
  CI では GitHub Secrets を利用して注入する

---

### 9.8 可用性・保守性

- RuboCop / RSpec / Vitest により品質を自動チェック
- GitHub Actions により CI を自動化し、
  - 規約違反や明確なバグを含むコードはマージ不可とする
